    ...
    <main className="min-h-screen bg-[#09090b] text-zinc-100">
      ...
      <div className="relative mx-auto max-w-6xl px-6">
              ...
              {hasQuery && (
                ...
                >
                  ...
                  <motion.div
                      ...
                      whileHover={{ y: -2 }}
                      whileTap={{ scale: 0.98 }}
                      onClick={() => searchYoutube(encodedQuery)}
                      className="rounded-full border border-zinc-700 bg-zinc-900/80 px-4 py-2 text-sm text-zinc-200 transition hover:border-zinc-500 hover:bg-zinc-800/80 hover:text-white"
                    >
                      Search on Youtube
                    </motion.div>
        ...
        <motion.div className="flex flex-col gap-4">
            {videos.map((video) => (
              <a href={`https://www.youtube.com/watch?v=${video.id}`} target="_blank" rel="noopener noreferrer" key={video.id} className="flex items-center gap-4">
                <Image src={video.thumbnail} alt={video.title} width={100} height={100} />
